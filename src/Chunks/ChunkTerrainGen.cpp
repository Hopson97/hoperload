...
std::vector<ChunkPosition> createChunkTerrain(ChunkMap& chunkmap, int chunkX, int chunkZ,
                                              int worldSize,
                                              const TerrainGenOptions& TerrainGenOptions)
{
    ...
    for (int y = 0; y < std::max(WATER_LEVEL / CHUNK_SIZE, maxHeight / CHUNK_SIZE) + 1;
         y++) {
