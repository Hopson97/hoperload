...
{
    // glBufferData
    glNamedBufferStorage(m_vbo, sizeof(Vertex) * verts.size(), verts.data(),
...
{
    // glBufferData
    glNamedBufferStorage(m_vbo, sizeof(VoxelVertex) * verts.size(), verts.data(),
                         GL_DYNAMIC_STORAGE_BIT);
    ...
    glVertexArrayAttribFormat(m_vao, 1, 3, GL_FLOAT, GL_FALSE,
                              offsetof(VoxelVertex, textureCoord));
    glVertexArrayAttribFormat(m_vao, 2, 1, GL_FLOAT, GL_FALSE,
                              offsetof(VoxelVertex, light));
...
{
    glNamedBufferStorage(m_ibo, sizeof(GLuint) * indices.size(), indices.data(),
                         GL_DYNAMIC_STORAGE_BIT);
